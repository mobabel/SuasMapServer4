...
function GeoXml(myvar, map, url, opts) {
  ...
  else {this.publishdirectory = "../plugin/geoxml/";}/*http://www.dyasdesigns.com/tntmap/*/
...
GeoXml.prototype.createMarkerJSON = function(item,idx) {
	//add try-catch block by lee, ie throw exception, can not find _type blabla....
	try{
		var that = this;
		...
		if(kid == "gml:boundedBy" || kid  == "boundedBy"){
			 for (var j = 0; j < n.childNodes.length; j++) {
				...
				if(nn == "Box" || nn == "gml:Box"){
					...
					//add try-catch block by lee, fix IE7 throw "ie can not open this page....."
					try{
