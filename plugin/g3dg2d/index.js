...
function DS_goDirections() {
	...
	/*  var route_points_loc = new Array();
  for(var i=0;i<route_points.length;i++){
	route_points_loc.push(new GLatLng(route_points[i][1], route_points[i][0]));
  }*/
	//var line = new GPolyline(route_points_loc, '#000080');
	//var stepDistance = line.getLength();
	//DS_map.addOverlay(line);
...
function DS_directionsLoaded() {
	...
	var route_points_start = route_points_array[0];
	var route_points_end = route_points_array[rp_length-1];
	var route_points_start_length = route_points_start.length;
	var route_points_end_length = route_points_end.length;

	reversegeocoder.reverseGeocode(new GLatLng(route_points_start[0][1],route_points_start[0][0]));
	var startAddr = currentLocAddress;
	reversegeocoder.reverseGeocode(new GLatLng(route_points_end[route_points_end_length-1][1]
	                             ,route_points_end[route_points_end_length-1][0]));
	var endAddr = currentLocAddress;
//	alert(startAddr + endAddr);
	...
	DS_placemarks['start'] = DS_geHelpers.createPointPlacemark(
			new google.maps.LatLng(route_points_start[0][1],route_points_start[0][0] ),
			{description: startAddr, standardIcon: 'grn-diamond'});
	...
	DS_placemarks['end'] = DS_geHelpers.createPointPlacemark(
			new google.maps.LatLng(
					route_points_end[route_points_end_length-1][1]
					,route_points_end[route_points_end_length-1][0]
			),
			{description: endAddr, standardIcon: 'grn-diamond'});
	...
	DS_flyToLatLng(new google.maps.LatLng(route_points_start[0][1],route_points_start[0][0]));
...
function DS_buildPathStepArrays() {
	...
	var numPolylineVertices ;
	var numSteps = rp_length;

	//route_points_array loop
	for (var i = 0; i < numSteps; i++) {
		route_points = route_points_array[i];
		var step = new GLatLng(route_points[0][1], route_points[0][0]);

		var firstPolylineIndex = 0;

		var lastPolylineIndex = route_points.length;

		numPolylineVertices = lastPolylineIndex;
		...
		var route_points_loc = new Array();
		for(var k=0;k<route_points.length;k++){
			route_points_loc.push(new GLatLng(route_points[k][1], route_points[k][0]));
		}
		var line = new GPolyline(route_points_loc, '#000080');
		var stepDistance = line.getLength();
		DS_map.addOverlay(line);

		for (var j = firstPolylineIndex; j < lastPolylineIndex-2; j++) {
			var loc = new GLatLng(route_points[j][1], route_points[j][0]);
			var distance = (j == numPolylineVertices - 1) ?
					0 : DS_geHelpers.distance(loc, new GLatLng(route_points[j+1][1], route_points[j+1][0]));
			...
			DS_path.push({
				...
				//TODO need get the sonsume time from db
				duration: 10/*100 * distance / stepDistance*/
			});
		}
	}
...
function DS_buildDirectionsList() {  
	...
	reversegeocoder.reverseGeocode(new GLatLng(route_points_start[0][1],route_points_start[0][0]));
	var startAddr = currentLocAddress;
	if(startAddr.length==0)startAddr = "Start";
	reversegeocoder.reverseGeocode(new GLatLng(route_points_end[route_points_end_length-1][1]
	                             ,route_points_end[route_points_end_length-1][0]));
	var endAddr = currentLocAddress;
	if(endAddr.length==0)endAddr = "End";
	
	$('#route-details').append($(
			'<div id="dir-start">' + startAddr + '</div>'));
	...
	$('#route-details').append($(
			'<div id="dir-end">' + endAddr + '</div>'));

	// handle events on the directions list
	$('#dir-start').click(function() {
		DS_flyToLatLng(new google.maps.LatLng(
				route_points[0][1],route_points[0][0]));
	});

	$('#dir-end').click(function() {
		DS_flyToLatLng(new google.maps.LatLng(
				route_points[route_points_length-1][1],route_points[route_points_length-1][0]));
	});
...
function DS_controlSimulator(command, opt_cb) {
	...
	case 'reset':
		...
		if (!DS_mapMarker) {
			...
			icon.image = '../plugin/g3dg2d/model/'+nav_type+'.png';
