...
if ($errornumber == 0) {
    ...
    $array_wh = array(0, 0, $map_paras['width'], $map_paras['height']);
    $pdf = &new Cezpdf($array_wh);
    $pdf->selectFont('www.ros.co.nz/fonts/Helvetica');

    $pdf->addInfo("Creator", SUAS_NAME . SITE_VERSION . SITE_VERSION_EDITION);
    $pdf->addInfo("Author", SUAS_NAME);
    $pdf->addInfo("Title", SUAS_NAME . " in PDF");
    $pdf->addInfo("Subject", $_SERVER['SERVER_NAME']);
    $pdf->addInfo("Keywords", SUAS_NAME . SITE_VERSION . SITE_VERSION_EDITION);

    $ezpdfrender->setRender($minx, $miny, $maxx, $maxy, $map_paras['width'], $map_paras['height'], $pdf, $enablestretchmap);
    if ($atlas_cfg['showCopyright']) {
        $pdf->setColor(1, 0, 0);
        $ezpdfrender->createTextWithScreenCoordinate(15, 15, "Copyright(C)" . SUAS_NAME . SITE_VERSION . SITE_VERSION_EDITION, 5, $color_copyrightinfo);
    }
            ...
            while ($line1_ = $database->getColumns($rs1_)) {
                ...
                switch ($data_Geom_Type) {
                    ...
                    case 'IMAGE': {
                            ...
                            $data_y = $wktparser->wktPointY;
                            $Number_Point = $wktparser->wktPointNr;
                            $ezpdfrender->createImage($data_x, $data_y, $Number_Point, $custom_image_path_absolute.$data_ImageLink, $xmlFillOpacity, $color_StrokeColor);
                        }
                        break;
                    // ======================================================================================================
                    // ======================================================================================================
    ...
    if(debugMode){
    	if(!$btn_user_interface){
	    	$fp = fopen('../files/atlas/'.$aid.'/cache/map.pdf', 'wb');
    	}else{
