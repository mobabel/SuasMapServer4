...
if ($atlas_cfg['enableCache']) {
    if (equalIgnoreCase($map_paras['format'], $MAP_RENDER_FORMAT['kml'])) {
        $cache = new Cache($atlas_cfg['cacheExpiredTime'], Cache::$TYPE_KML, $aid);
        $cache->cacheCheck();
    }
	if (equalIgnoreCase($map_paras['format'], $MAP_RENDER_FORMAT['kmz'])) {
        $cache = new Cache($atlas_cfg['cacheExpiredTime'], Cache::$TYPE_KMZ, $aid);
        $cache->cacheCheck();
    }
}
...
if ($errornumber == 0) {
    ...
    if(equalIgnoreCase($map_paras['format'], $MAP_RENDER_FORMAT['kml'])){
        header("Content-type: ".$MAP_RENDER_FORMAT['kml']);
        $useStream = 1;
    }
    if(equalIgnoreCase($map_paras['format'], $MAP_RENDER_FORMAT['kmz'])){
        header("Content-type: ". $MAP_RENDER_FORMAT['kmz']);
	    $useStream = 0;
	}
    ...
    $kml->setRender($minx, $miny, $maxx, $maxy, $map_paras['width'], $map_paras['height'], $useStream,
        $map_paras['poix'], $map_paras['poiy'], $map_paras['poiz'], $map_paras['pitch'] , $map_paras['yaw'], 
        	$map_paras['roll'], $map_paras['distance'], $map_paras['aov'], $enablestretchmap, $use3d);
