...
if ($errornumber == 0) {
    ...
    /*if ( equalIgnoreCase($map_paras['format'], $MAP_RENDER_FORMAT['svgtz']) ){
    	header("Content-Type: ".$MAP_RENDER_FORMAT['svgtz']);
    }else if(equalIgnoreCase($map_paras['format'], $MAP_RENDER_FORMAT['svgbz'])){
		header("Content-Type: ".$MAP_RENDER_FORMAT['svgbz']);
	}*/
	
	//dont output now, use the result
    ...
    for ($ii = 0; $ii < $numberofvalueslayer; $ii++) {
        ...
        */
        ...
        if ($showlayer) {
            ...
            $rs1_ = $database->getGeomAsTextInBboxBySrsLayer($tbname, $minx, $miny, $maxx, $maxy, $map_paras['srs'], $layersvalues[$ii]);
