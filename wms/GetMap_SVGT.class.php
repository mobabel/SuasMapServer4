...
if ($errornumber == 0) {
    ...
    */
	...
	/*if ( equalIgnoreCase($map_paras['format'], $MAP_RENDER_FORMAT['svgt']) ){
    	header("Content-Type: ".$MAP_RENDER_FORMAT['svgt']);
    }else if(equalIgnoreCase($map_paras['format'], $MAP_RENDER_FORMAT['svgb'])){
		header("Content-Type: ".$MAP_RENDER_FORMAT['svgb']);
	}*/
    ...
    if (!$atlas_cfg['enableStreamSVG']) {
        ...
        $svgz = new SVGTStreamRender($atlas_cfg['enableStreamSVG']);
        $svgz->setSVGTStreamRender($minx, $miny, $maxx, $maxy, $map_paras['width'], $map_paras['height'], $enablestretchmap, $atlas_cfg['enableSVGPixelCoordinate']);
                ...
                switch ($data_Geom_Type) {
                    ...
                    case 'IMAGE': {
                            ...
                            if (!$atlas_cfg['enableStreamSVG']) {
                                ...
                                $svgimage[$flagImage] = new SvgImage($data_Recid, $data_x[0], - $data_y[0], $data_w, $data_h, $custom_image_path.$data_ImageLink, $style4eachlayer, "");
